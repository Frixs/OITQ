...
function OnPlayerDeathLoot( hero )
    ...
    if randomNumber < 0 and dropped == false then  -- 0.0% chance (common) -- OFF
        ...
        -- uncommon
        randomNumber = RandomInt( 1, lootKV['percentageLevel'] )
        if randomNumber < 35 and dropped == false then  -- 3.5% chance (uncommon)
            ChooseRandomItem( hero, "uncommon", lootKV )
            dropped = true
        else
            -- rare
            randomNumber = RandomInt( 1, lootKV['percentageLevel'] )
            if randomNumber < 25 and dropped == false then  -- 2.5% chance (rare)
                ChooseRandomItem( hero, "rare", lootKV )
                dropped = true
            else
                -- epic
                randomNumber = RandomInt( 1, lootKV['percentageLevel'] )
                if randomNumber < 15 and dropped == false then  -- 1.5% chance (epic)
                    ChooseRandomItem( hero, "epic", lootKV )
                    dropped = true
                else
                    -- artifact
                    randomNumber = RandomInt( 1, lootKV['percentageLevel'] )
                    if randomNumber < 10 and dropped == false then  -- 1.0% chance (artifact)
