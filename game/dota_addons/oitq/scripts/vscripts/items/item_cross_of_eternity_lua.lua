function item_cross_of_eternity_on_spell_start( keys )	
	...
	for i=0,5 do
		...
		if opposite_item == "item_hazzard_goggles" then
		...
		if caster:HasModifier("modifier_item_hazzard_goggles_charge_1") then
			if caster:HasModifier("modifier_item_hazzard_goggles_charge_2") then
				if caster:HasModifier("modifier_item_hazzard_goggles_charge_3") then
					caster:RemoveModifierByName("modifier_item_hazzard_goggles_charge_3")
					if opposite_item ~= nil then opposite_item:SetCurrentCharges(2) end
				else
					caster:RemoveModifierByName("modifier_item_hazzard_goggles_charge_2")
					if opposite_item ~= nil then opposite_item:SetCurrentCharges(1) end
				end
			else
				caster:RemoveModifierByName("modifier_item_hazzard_goggles_charge_1")
				if opposite_item ~= nil then opposite_item:SetCurrentCharges(0) end
			end
		else
			item_ability:ApplyDataDrivenModifier(caster, caster, "modifier_item_cross_of_eternity_charge_"..(current_charges+1), {})
			item_ability:SetCurrentCharges(current_charges + 1)
		end
	end
end

function item_cross_of_eternity_on_death( keys )
	local caster 	   = keys.caster
	local item_ability = keys.ability
	if item_ability == nil then  -- this is because of drop item and then pickup to inventory
		for i=0,5 do
			...
			if item_ability == "item_cross_of_eternity" then
				item_ability = caster:GetItemInSlot( i )
				break
			end
		end
...
function modifier_item_cross_of_eternity_damage_cooldown_on_take_damage( keys )
...
function item_cross_of_eternity_on_equip( keys )
	local caster = keys.caster
	local item_ability = keys.ability
